...
func UpdateUserProfileHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		updateUser, err := service.UpdateUserProfile(db, id, userUpdate.Username, userUpdate.Email)
...
func UpdateUserProfilePictureHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err = service.UpdateProfilePicture(db, id, filename)
...
func DeleteUserByIdHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Delete User
		err = service.DeleteUser(c, db, id)
		if err != nil {
...
func ChangePasswordHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		userID := c.GetInt("userID")
		err := service.ChangePassword(db, userID, req.CurrentPassword, req.NewPassword)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
